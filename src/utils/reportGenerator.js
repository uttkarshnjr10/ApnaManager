    ...
    try {
        ...
        for (const guest of guests) {
            ...
            const fullAddress = [
                ...
                address.state,
                address.zipCode,
            ].filter(Boolean).join(', '); // Join address parts with a comma

            const checkIn = new Date(stay.checkIn).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' });
            const expectedCheckout = new Date(stay.expectedCheckout).toLocaleString('en-IN', { dateStyle: 'short' });
