...
const registerUser = asyncHandler(async (req, res) => {
    ...
    let user;
    if (role === 'Hotel') {
        user = await HotelUser.create({ ...commonData, ...details });
    } else if (role === 'Police') {
...
const getAdminDashboardData = asyncHandler(async (req, res) => {
    ...
    const recentHotels = await HotelUser.find()
        .sort({ createdAt: -1 })
        .limit(5)
        .select('username city status');

    const recentPolice = await PoliceUser.find()
        .sort({ createdAt: -1 })
        .limit(5)
        .select('username jurisdiction status');
...
const getHotelUsers = asyncHandler(async (req, res) => {
    ...
    const hotels = await HotelUser.find(query).select('-password');
    res
    .status(200)
...
const getPoliceUsers = asyncHandler(async (req, res) => {
    ...
    const policeUsers = await PoliceUser.find(query).select('-password');
    res
    .status(200)
